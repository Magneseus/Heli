...
bool OgreRay::RaycastFromPoint(const Ogre::Vector3& point, const Ogre::Vector3& normal, Ogre::Vector3& result)
{
	...
	return RaycastFromPoint(point, normal, result, L);
}
...
bool OgreRay::RaycastFromPoint(const Ogre::Vector3& point, const Ogre::Vector3& normal, Ogre::Vector3& result, std::vector<Ogre::MovableObject*>& ignoreList, Ogre::MovableObject* &resultObj)
{
	...
	for (size_t qr_idx = 0, size = query_result.size(); qr_idx < size; ++qr_idx)
	{
		...
		bool checkResult = (query_result[qr_idx].movable != NULL);

		if (checkResult)
		{
			for (auto it = ignoreList.begin(); it != ignoreList.end(); ++it)
			{
				if (query_result[qr_idx].movable == *it)
				{
					checkResult = false;
					break;
				}
		...
		// only check this result if its a hit against an entity && it's not in the ignore list
		if (checkResult)
		{
